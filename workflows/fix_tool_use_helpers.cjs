...
// New tool code using $helpers.httpRequest instead of fetch
// Also wraps everything in try-catch for better error visibility
...
try {
  // Critère passé par l'agent IA
  const q = (query || '').toLowerCase().trim();

  // Tous les champs disponibles dans la table locaux
  const SELECT_FIELDS = [
  ...
  // Note: disponible stocke "true"/"false" (n8n v2) et "Oui"/"Non" (ancien)
  const queryParams = {};

  // Filtre disponible: exclure Non et false
  queryParams['not.or'] = '(disponible.eq.Non,disponible.eq.false)';
  ...
  if (communeTrouvee) queryParams['commune'] = 'ilike.*' + communeTrouvee.value + '*';
  ...
  if (typeTrouve) queryParams['type_de_bien'] = 'ilike.*' + typeTrouve + '*';

  // Filtre type d'offre
  if (q.includes('vente') || q.includes('vendre') || q.includes('achat') || q.includes('acheter') || q.includes('céder')) {
    queryParams['type_offre'] = 'eq.Vente';
  } else if (q.includes('location') || q.includes('louer') || q.includes('locat') || q.includes('bail') || q.includes('mensuel')) {
    queryParams['type_offre'] = 'eq.Location';
  }

  // Filtre meuble
  if (q.includes('meublé') || q.includes('meuble')) queryParams['meubles'] = 'eq.true';

  // Filtre REF bien
  const refMatch = q.match(/([a-z]{2,3}[-_][a-z]{2,3}[-_][a-zA-Z0-9]{4,10})/i);
  if (refMatch) queryParams['ref_bien'] = 'ilike.*' + refMatch[1].toUpperCase() + '*';

  // Filtre quartier
  const quartiersConnus = ['angré', 'angre', '2 plateaux', 'deux plateaux', 'riviera', 'bonoumin', 'attoban', 'niangon', 'orly', 'zone 4', 'zone4', 'solibra', 'remblai'];
  const quartierTrouve = quartiersConnus.find(qt => q.includes(qt));
  if (quartierTrouve) queryParams['quartier'] = 'ilike.*' + quartierTrouve + '*';

  // Filtre nombre de chambres
  const chambreMatch = q.match(/(\\d+)\\s*(?:pièces?|chambres?|ch\\b)/);
  if (chambreMatch) queryParams['chambre'] = 'eq.' + chambreMatch[1];

  // Ajouter select, order, limit
  queryParams['select'] = SELECT_FIELDS;
  queryParams['order'] = 'date_publication.desc';
  queryParams['limit'] = '15';

  // Construire URL avec les paramètres
  const paramStr = Object.entries(queryParams).map(([k, v]) => k + '=' + v).join('&');
  const url = SUPABASE_URL + '/rest/v1/locaux?' + paramStr;

  // Exécuter la requête avec $helpers.httpRequest (méthode n8n native)
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    return 'Erreur Supabase ' + response.statusCode + ': ' + JSON.stringify(response.body).substring(0, 200);
  }

  const biens = response.body;

  if (!biens || biens.length === 0) {
    return 'Aucun bien immobilier disponible ne correspond à votre recherche en ce moment. Essayez avec des critères différents (autre commune, autre type de bien...).';
  }
  ...
  return 'Exception dans l\\'outil: ' + err.message + ' | Stack: ' + (err.stack || '').substring(0, 200);
}`;
...
(async function() {
  ...
  console.log('New code length:', NEW_TOOL_CODE.length);
  console.log('Uses $helpers.httpRequest:', NEW_TOOL_CODE.includes('$helpers.httpRequest'));
  console.log('Has try-catch:', NEW_TOOL_CODE.includes('try {'));
  ...
  if (put.body && put.body.id) {
    ...
    const hasHelpers = check && check.parameters && check.parameters.jsCode &&
      check.parameters.jsCode.includes('$helpers.httpRequest');
    console.log('$helpers.httpRequest deployed:', hasHelpers ? 'YES' : 'NO');
